...
- (void)initialize
{   
    _appearance = [[[FSCalendarAppearance alloc] init] retain];
    _appearance.calendar = self;
    
    _gregorian = [[[NSCalendar alloc] initWithCalendarIdentifier:NSCalendarIdentifierGregorian] retain];
    ...
    _today = [[self.gregorian dateBySettingHour:0 minute:0 second:0 ofDate:[NSDate date] options:0] retain];
    _currentPage = [[self.gregorian fs_firstDayOfMonth:_today] retain];
    
    
    _minimumDate = [[self.formatter dateFromString:@"1970-01-01"]retain];
    _maximumDate = [[self.formatter dateFromString:@"2099-12-31"]retain];
    ...
    _selectedDates = [[NSMutableArray arrayWithCapacity:1] retain];
...
{
    _visibleSectionHeaders = [NSMapTable weakToWeakObjectsMapTable];
    return [[self.visibleSectionHeaders.dictionaryRepresentation allValues] retain];
}
