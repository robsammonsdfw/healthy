...
//	[view addSubview:hud];
    dispatch_async(dispatch_get_main_queue(), ^{ [view addSubview:hud]; });
	[hud show:animated];
#if __has_feature(objc_arc)
	return hud;
#else
...
//			CGSize labelSize = [detailsLabel.text sizeWithFont:detailsLabel.font constrainedToSize:CGSizeMake(frame.size.width - 4*margin, maxHeight) lineBreakMode:detailsLabel.lineBreakMode];
            ...
            CGRect textRect = [detailsLabel.text boundingRectWithSize:CGSizeMake(frame.size.width - 4*margin, maxHeight)
